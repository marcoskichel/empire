...
| [`empire-git`](plugins/empire-git/README.md)           | Git workflow: parallel worktree lifecycle (`open`, `close`, `merge`, `list`, `cleanup`, `help`), `pr-description` and `pr-comment-reply` templating, and gated `pr-merge` / `pr-merge-stack`. |
| [`empire-dev`](plugins/empire-dev/README.md)           | Code `team-review`, engineering diagnostics (`shape`, `weigh`, `slice`), plus 13 bundled dev subagents (generalist review, paradigms, domain experts).                                        |
